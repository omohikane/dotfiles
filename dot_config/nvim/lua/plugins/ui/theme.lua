...
-- This file handles the orchestration of themes and highlighting adjustments
...
vim.api.nvim_create_user_command("ThemeNext", function()
	idx = (idx % #THEMES) + 1
	require("lazy").load({ plugins = { THEMES[idx].plugin } })
	apply(THEMES[idx].scheme)
...
vim.schedule(function()
	require("lazy").load({ plugins = { DEFAULT.plugin } })
	apply(DEFAULT.scheme)
	apply_highlights()
end)

-- This file acts as a spec that just performs side effects
return {}
